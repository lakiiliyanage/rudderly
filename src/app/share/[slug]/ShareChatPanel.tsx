...
export default function ShareChatPanel({
  agentId,
  agentName,
}: {
  agentId:   string
  ...
  async function handleSend() {
    ...
    try {
      const res = await fetch(`/api/share/${agentId}/chat`, {
        method:  'POST',
  ...
  function handleNewConversation() {
    if (messages.length > 0 && !window.confirm('Start a new conversation? This will clear the current chat.')) return
    abortRef.current?.abort()
    setMessages([])
    setError(null)
    setInterrupted(false)
  ...
  return (
    <div className="bg-gray-900 border border-gray-800 rounded-2xl overflow-hidden flex flex-col h-[560px]">
      ...
      {/* ── Privacy banner ─────────────────────────────────────────────── */}
      <div className="mx-4 mt-3 px-3.5 py-2.5 bg-blue-950/40 border border-blue-800/40 rounded-xl text-xs text-blue-300 shrink-0 flex items-center gap-2">
        <svg className="w-3.5 h-3.5 shrink-0" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" d="M16.5 10.5V6.75a4.5 4.5 0 10-9 0v3.75m-.75 11.25h10.5a2.25 2.25 0 002.25-2.25v-6.75a2.25 2.25 0 00-2.25-2.25H6.75a2.25 2.25 0 00-2.25 2.25v6.75a2.25 2.25 0 002.25 2.25z" />
        </svg>
        🔒 This is a public demo. Your messages are not saved.
      </div>
              ...
              <div className="max-w-[75%] rounded-2xl rounded-bl-sm px-4 py-2.5 text-sm leading-relaxed break-words bg-gray-800 text-gray-200">
                ...
                {msg.sources && msg.sources.length > 0 && (
                  <div className="mt-2 pt-2 border-t border-gray-700/60">
                    ...
                    >
                      <span>Sources</span>
                      <span className={`transition-transform duration-150 ${expandedSources.has(i) ? 'rotate-90' : ''}`}>›</span>
                    </button>
      ...
      <div className="px-4 py-4 border-t border-gray-800 shrink-0">
        ...
        <div className="flex items-center justify-between mt-2 px-1">
          {input.trim().length > 4000 ? (
            <p className="text-xs text-yellow-500/80">Your message is very long — this may affect response quality.</p>
          ) : (
